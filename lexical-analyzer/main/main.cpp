...
//    }
    ...
    std::vector<Token *> v;
    NFA *nfa = new NFA();
    int root = nfa->createNode();
    int A = nfa->createNode();
    int B = nfa->createNode();
    int C = nfa->createNode();
    int D = nfa->createNode();
    nfa->addTransition(EPS_TRANS, root, A);
    nfa->addTransition(EPS_TRANS, root, B);
    nfa->addTransition('1',  A, D);
    nfa->addTransition('0',  A, B);
    nfa->addTransition('1',  A, C);
    nfa->addTransition('0',  A, C);
    nfa->addTransition('1',  B, D);
    nfa->addTransition('1',  C, C);
    nfa->addTransition('0',  C, B);
    nfa->addTransition('0',  C, A);
    nfa->addTransition('1',  D, A);
    nfa->addTransition('1',  D, C);
    nfa->setStateSpec(D,1, "accepted",ACCEPTED);
    std::cout<<"==========="<<std::endl;
