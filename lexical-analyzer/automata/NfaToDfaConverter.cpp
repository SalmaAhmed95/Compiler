    ...
    while (!queueDfaStates.empty()) {
        ...
        for (auto transition : nfa->getAllAttributes()) {
            if (transition != EPS_TRANS) {
                ...
                getNextState(&newStates, &curNode.nfaStates, &epsTable, &transitionTable, transition, charIndex,
                             nfa);
...
//        std::cout<<transitionTable->size()<< "  "<<(*transitionTable)[0].size()<<" \n";
//        std::cout<<nfaState<<" "<<charIndex<<"  \n";
        if ((*transitionTable)[nfaState][charIndex].stateSpec.stateType == INTERMEDIATE &&
            (*transitionTable)[nfaState][charIndex].states.empty()) {
//            std::cout << "inside if\n";
            getNextStateForChar(&(*transitionTable)[nfaState][charIndex], nfaState, epsTable, transition,
                                nfa);
        }
        for (auto nextState : (*transitionTable)[nfaState][charIndex].states) {
            nextStates->states.insert(nextState);
        }
        updateSetOfNfaStatesSpec(&nextStates->stateSpec, (*transitionTable)[nfaState][charIndex].stateSpec.stateType,
...
void NfaToDfaConverter::getNextStateForChar(SetOfNfaStates *nextStates,
                                            stateID curNfaState,
                                            std::vector<SetOfNfaStates> *epsTable,
...
NfaToDfaConverter::constructTransitionTable(NFA *nfa) {
    std::vector<std::vector<SetOfNfaStates> > transitionTable(nfa->getNumberOfStates());
